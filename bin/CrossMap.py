...
from cmmodule import (BED, annoGene, bgrMerge, ireader,
                      myutils, sam_header)
...
def read_chain_file(chain_file, print_table=False):
    ...
    for line in ireader.reader(chain_file):
        # Example: chain 4900 chrY 58368225 + 25985403 25985638 chr5 151006098
        # - 43257292 43257528 1
        if not line.strip():
            continue
        line = line.strip()
        ...
        if fields[0] == 'chain' and len(fields) in [12, 13]:
            ...
            if source_strand != '+':
                raise Exception(
                    "Source strand in an .over.chain file must be +. (%s)" % line)
            ...
            if target_strand == '+':
                maps[source_name].add_interval(Interval(
                    sfrom, sfrom + size, (target_name, tfrom, tfrom + size, target_strand)))
            elif target_strand == '-':
                maps[source_name].add_interval(Interval(
                    sfrom, sfrom + size,
                    (target_name, target_size - (tfrom + size), target_size - tfrom, target_strand)))
...
def crossmap_vcf_file(mapping, infile, outfile, liftoverfile, refgenome):
    ...
    for line in ireader.reader(infile):
        ...
        if fields[0].startswith('#'):
            print >> FILE_OUT, "##liftOverProgram=CrossMap(https://github.com/huangzhibo/CrossMap)"
...
def crossmap_bed_file(mapping, inbed, outfile=None):
    ...
    for line in ireader.reader(inbed):
        ...
        if len(fields) < 12:
            ...
            a = map_coordinates(mapping, fields[0], int(
                fields[1]), int(fields[2]), strand)
            # example of a: [('chr1', 246974830, 246974833,'+'), ('chr1',
            # 248908207, 248908210,'+')]
            ...
            try:
                ...
                if len(a) > 2:
                    ...
                    for j in range(1, len(a), 2):
                        ...
                        if outfile is None:
                            print line + '\t' + '(split.' + str(count) + ':' + ':'.join(
                                [str(i) for i in a[j - 1]]) + ')\t' + '\t'.join([str(i) for i in fields])
                        else:
                            print >> FILE_OUT, '\t'.join(
                                [str(i) for i in fields])
            except:
                if outfile is None:
                    print line + '\tFail'
                else:
        ...
        if len(fields) == 12 or len(fields) == 20:
            ...
            for e_chr, e_start, e_end in exons_old_pos:
                ...
                if len(a) == 2:
                    ...
                    # a has two elements, first is query, 2nd is target. #
                    # [('chr1', 246974830, 246974833,'+'), ('chr1', 248908207,
                    # 248908210,'+')]
                else:
                    fail_flag = True
                    break
            ...
            if not fail_flag:
                # check if all exons were mapped to the same chromosome the
                # same strand
                chr_id = set()
                exon_strand = set()
                ...
                if not fail_flag:
                    ...
                    new_bedline = '\t'.join(
                        str(i) for i in (new_chrom, new_chrom_st, new_chrom_end, new_name, new_score,
                                         new_strand, new_thickStart, new_thickEnd, new_ittemRgb, new_blockCount,
                                         new_blockSizes, new_blockStarts))
...
def crossmap_bam_file(mapping, chainfile, infile, outfile_prefix, chrom_size, IS_size=200, IS_std=30, fold=3,
    ...
    (new_header, name_to_id) = sam_header.bam_header_generator(orig_header=sam_ori_header, chrom_size=chrom_size,
                                                               prog_name="CrossMap", prog_ver=__version__,
                                                               format_ver=1.0, sort_type='coordinate', co=comments)

    #
    if outfile_prefix is not None:
        if bam_format:
            OUT_FILE = pysam.Samfile(
                outfile_prefix + '.bam', "wb", header=new_header)
            # OUT_FILE_UNMAP = pysam.Samfile( outfile_prefix + '.unmap.bam', "wb", template=samfile )
            printlog(["Liftover BAM file:", infile,
                      '==>', outfile_prefix + '.bam'])
        else:
            OUT_FILE = pysam.Samfile(
                outfile_prefix + '.sam', "wh", header=new_header)
    ...
    try:
        while (1):
            ...
            # qcfailed reads will be written to OUT_FILE_UNMAP for both SE and
            # PE reads
            if old_alignment.is_qcfail:
                QF += 1
                if addtag:
                    old_alignment.set_tag(tag="QF", value=0)
            ...
            # new_alignment_aend = old_alignment.aend         # reference End
            # position of the aligned part (of read)
            ...
            new_alignment.qname = old_alignment.qname  # 1st column. read name.
            # 10th column. read sequence. all bases.
            new_alignment.seq = old_alignment.seq
            # 11th column. read sequence quality. all bases.
            ...
            if old_alignment.is_paired:
                ...
                if old_alignment.is_unmapped:
                    ...
                    if old_alignment.mate_is_unmapped:
                        NN += 1
                        if addtag:
                            old_alignment.set_tag(tag="NN", value=0)
                        OUT_FILE.write(old_alignment)
                        continue
                        ...
                        try:
                            ...
                            read2_maps = map_coordinates(
                                mapping, read2_chr, read2_start, read2_end, read2_strand)
                            # [('chr1', 246974830, 246974833, '+' ), ('chr1', 248908207, 248908210, '+' )]
                        except:
                            read2_maps = None
                    ...
                    # ------------------------------------
                        ...
                        # 3
                        # recommend to set the RNAME of unmapped read to its
                        # mate's
                        new_alignment.tid = name_to_id[read2_maps[1][0]]
                        # 4
                        # recommend to set the POS of unmapped read to its
                        # mate's
                        ...
                        NU += 1
                        if addtag:
                            new_alignment.set_tag(tag="NU", value=0)
                    ...
                    # ------------------------------------
                        ...
                        NM += 1
                        if addtag:
                            new_alignment.set_tag(tag="NM", value=0)
                        OUT_FILE.write(new_alignment)
                        continue
                    ...
                    try:
                        ...
                        read1_maps = map_coordinates(
                            mapping, read1_chr, read1_start, read1_end, read1_strand)
                    except:
                        read1_maps = None
                ...
                if read1_maps is None:
                    ...
                    if old_alignment.mate_is_unmapped:
                        NN += 1
                        if addtag:
                            new_alignment.set_tag(tag="NN", value=0)
                        OUT_FILE.write(new_alignment)
                        continue
                    ...
                    if read1_maps[0][3] != read1_maps[1][3]:  # opposite strand
                        ...
                        new_alignment.seq = revcomp_DNA(old_alignment.seq)
                        # 11
                        new_alignment.qual = old_alignment.qual[
                    ...
                    if (old_alignment.mate_is_unmapped) or (read2_maps is None):
                        ...
                        UN += 1
                        if addtag:
                            new_alignment.set_tag(tag="UN", value=0)
                        OUT_FILE.write(new_alignment)
                        continue
                    ...
                    # (2) R2 is unique mapped
                        ...
                        # 9
                        new_alignment.tlen = abs(
                            new_alignment.pos - new_alignment.pnext) - 1 - old_alignment.alen
                        # 2
                        if (read2_maps[1][3] != read1_maps[1][3]) and (
                                    new_alignment.tlen <= IS_size + fold * IS_std) and (
                                    new_alignment.tlen >= IS_size - fold * IS_std):
                            new_alignment.flag = new_alignment.flag | 0x2

                        UU += 1
                        if addtag:
                            new_alignment.set_tag(tag="UU", value=0)
                        OUT_FILE.write(new_alignment)
                        continue
                    ...
                    # (3) R2 is multiple mapped
                        ...
                        UM += 1
                        if addtag:
                            new_alignment.set_tag(tag="UM", value=0)
                        OUT_FILE.write(new_alignment)
                        continue
                    ...
                    if (old_alignment.mate_is_unmapped) or (read2_maps is None):
                        ...
                        MN += 1
                        if addtag:
                            new_alignment.set_tag(tag="MN", value=0)
                        OUT_FILE.write(new_alignment)
                        continue
                    ...
                    # (2) read2 is unique mapped
                        ...
                        MU += 1
                        if addtag:
                            new_alignment.set_tag(tag="MU", value=0)
                        OUT_FILE.write(new_alignment)
                        continue
                    ...
                    # (3) R2 is multiple mapped
                        ...
                        MM += 1
                        if addtag:
                            new_alignment.set_tag(tag="MM", value=0)
                        OUT_FILE.write(new_alignment)
                        continue
                else:
            ...
            # Singel end sequencing
                ...
                if old_alignment.is_unmapped:
                    SN += 1
                    if addtag:
                        old_alignment.set_tag(tag="SN", value=0)
                    OUT_FILE.write(old_alignment)
                    continue
                else:
                    ...
                    read_maps = map_coordinates(
                        mapping, read_chr, read_start, read_end, read_strand)
                ...
                if read_maps is None:
                    ...
                    SN += 1
                    if addtag:
                        new_alignment.set_tag(tag="SN", value=0)
                    OUT_FILE.write(new_alignment)
                    continue
                ...
                if len(read_maps) == 2:
                    ...
                    SU += 1
                    if addtag:
                        new_alignment.set_tag(tag="SU", value=0)
                    OUT_FILE.write(new_alignment)
                    continue
                ...
                if len(read_maps) > 2 and len(read_maps) % 2 == 0:
                    ...
                    SM += 1
                    if addtag:
                        new_alignment.set_tag(tag="SM", value=0)
                    OUT_FILE.write(new_alignment)
                    continue
    ...
    if outfile_prefix is not None:
        if bam_format:
            try:
                ...
                printlog(
                    ["Warning: ", "output BAM file was NOT sorted and indexed."])
...
def crossmap_wig_file(mapping, in_file, out_prefix, source_chrom_size, taget_chrom_size, in_format, binSize=100000):
    ...
    if in_format.upper() == "WIGGLE":
        ...
        for chr, start, end, strand, score in wiggleReader(in_file):
            ...
            # map_coordinates(mapping, q_chr, q_start, q_end, q_strand = '+',
            # print_match=False):
            maps = map_coordinates(mapping, chr, start, end, '+')
            if maps is None:
                continue
            if len(maps) == 2:
                print >> OUT_FILE1, '\t'.join(
                    [str(i) for i in [maps[1][0], maps[1][1], maps[1][2], score]])
            else:
                continue
            maps[:] = []
        OUT_FILE1.close()
        ...
        for (chr, start, end, score) in bgrMerge.merge(out_prefix + '.bgr'):
            print >> OUT_FILE2, '\t'.join(
                [str(i) for i in (chr, start, end, score)])
        OUT_FILE2.close()
        ...
        if wigToBigWig_cmd is not None:
            ...
            CS.close()
            try:
                subprocess.call([wigToBigWig_cmd, "-clip", out_prefix +
                                 '.sorted.bgr', tmp_file, out_prefix + '.bw'])
                # print ' '.join([wigToBigWig_cmd, "-clip",  out_prefix +
                # '.3.bgr', 'tmp.chromSizes', out_prefix + '.bw'])
            except:
                printlog(["Failed!"])
            try:
                os.remove(tmp_file)
            ...
            printlog(
                'Could not find wigToBigWig executable. Try to download it from "http://hgdownload.cse.ucsc.edu/admin/exe/" and convert wig to bigwig by yourself.')
...
def bed_help():
    msg = [
        ...
        ('Description:',
         "\"input_chain_file\" and \"input_bed_file\" can be regular or compressed (*.gz, *.Z, *.z, *.bz, *.bz2, *.bzip2) file, local file or URL (http://, https://, ftp://) pointing to remote file. BED format file must have at least 3 columns (chrom, start, end) and no more than 12 columns. If  no \"output_file\" is specified, output will be directed to the screen (console). BED format: http://genome.ucsc.edu/FAQ/FAQformat.html#format1"),
        ('Example:',
         "CrossMapy.py bed hg18ToHg19.over.chain.gz test.hg18.bed test.hg19.bed  # write output to \"test.hg19.bed\""),
        ('Example:',
         "CrossMapy.py bed hg18ToHg19.over.chain.gz test.hg18.bed  				 # write output to screen"),
    ]
    for i, j in msg:
        print >> sys.stderr, '\n' + i + '\n' + \
...
def gff_help():
    msg = [
        ...
        ('Description:',
         "\"input_chain_file\" can be regular or compressed (*.gz, *.Z, *.z, *.bz, *.bz2, *.bzip2) file, local file or URL (http://, https://, ftp://) pointing to remote file. Input file must be in GFF or GTF format. GFF format: http://genome.ucsc.edu/FAQ/FAQformat.html#format3 GTF format: http://genome.ucsc.edu/FAQ/FAQformat.html#format4"),
        ('Example:',
         "CrossMap.py gff  hg19ToHg18.over.chain.gz test.hg19.gtf test.hg18.gtf  # write output to \"test.hg18.gff\""),
...
def wig_help():
    msg = [
        ...
        ('Description:',
         "\"input_chain_file\" can be regular or compressed (*.gz, *.Z, *.z, *.bz, *.bz2, *.bzip2) file, local file or URL (http://, https://, ftp://) pointing to remote file.  Both \"variableStep\" and \"fixedStep\" wiggle lines are supported. Wiggle format: http://genome.ucsc.edu/goldenPath/help/wiggle.html"),
...
def bigwig_help():
    msg = [
        ...
        ('Description:',
         "\"input_chain_file\" can be regular or compressed (*.gz, *.Z, *.z, *.bz, *.bz2, *.bzip2) file, local file or URL (http://, https://, ftp://) pointing to remote file. Bigwig format: http://genome.ucsc.edu/goldenPath/help/bigWig.html"),
...
def vcf_help():
    msg = [
        ...
        ("Description:",
         "\"input_chain_file\" and \"input_VCF_file\" can be regular or compressed (*.gz, *.Z, *.z, *.bz, *.bz2, *.bzip2) file, local file or URL (http://, https://, ftp://) pointing to remote file. \"ref_genome_file\" is genome sequence file of 'target assembly' in FASTA format."),
        ...
        if sys.argv[1].lower() == 'bed':
            ...
            if len(sys.argv) == 5:
                ...
                (mapTree, targetChromSizes,
                 sourceChromSizes) = read_chain_file(chain_file)
                crossmap_wig_file(
                    mapTree, in_file, out_file, sourceChromSizes, targetChromSizes, in_format='wiggle')
            else:
                wig_help()
                sys.exit(0)
            ...
            if len(sys.argv) == 5:
                ...
                (mapTree, targetChromSizes,
                 sourceChromSizes) = read_chain_file(chain_file)
                crossmap_wig_file(
                    mapTree, in_file, out_file, sourceChromSizes, targetChromSizes, in_format='bigwig')
            else:
                bigwig_help()
                sys.exit(0)
        elif sys.argv[1].lower() == 'bam':
            # def crossmap_bam_file(mapping, chainfile, infile,
            # outfile_prefix, chrom_size, IS_size=200, IS_std=30, fold=3):
            usage = "CrossMap.py bam input_chain_file input_bam_file output_file [options]\nNote: If output_file == STDOUT or -, CrossMap will write BAM file to the screen"
            parser = optparse.OptionParser(usage, add_help_option=False)
            parser.add_option("-m", "--mean", action="store", type="float", dest="insert_size",
                              default=200.0, help="Average insert size of pair-end sequencing (bp). [default=%default]")
            parser.add_option("-s", "--stdev", action="store", type="float", dest="insert_size_stdev",
                              default=30.0, help="Stanadard deviation of insert size. [default=%default]")
            parser.add_option("-t", "--times", action="store", type="float", dest="insert_size_fold", default=3.0,
                              help="A mapped pair is considered as \"proper pair\" if both ends mapped to different strand and the distance between them is less then '-t' * stdev from the mean. [default=%default]")
            parser.add_option("-a", "--append-tags", action="store_true",
                              dest="add_tags", help="Add tag to each alignment.")
            (options, args) = parser.parse_args()

            if len(args) >= 3:
                print >> sys.stderr, "Insert size = %f" % (options.insert_size)
                print >> sys.stderr, "Insert size stdev = %f" % (
                    options.insert_size_stdev)
                print >> sys.stderr, "Number of stdev from the mean = %f" % (
                    options.insert_size_fold)
                if options.add_tags:
                    print >> sys.stderr, "Add tags to each alignment = %s" % (
                        options.add_tags)
                else:
                    print >> sys.stderr, "Add tags to each alignment = %s" % (
                        False)
                ...
                if options.add_tags:
                    crossmap_bam_file(mapping=mapTree, chainfile=chain_file, infile=in_file, outfile_prefix=out_file,
                                      ...
                                      IS_size=options.insert_size, IS_std=options.insert_size_stdev,
            ...
            if len(sys.argv) == 6:
                ...
                (mapTree, targetChromSizes,
                 sourceChromSizes) = read_chain_file(chain_file)
                crossmap_vcf_file(mapping=mapTree, infile=in_file, outfile=out_file,
                                  liftoverfile=sys.argv[2], refgenome=genome_file)
            else:
                vcf_help()
                sys.exit(0)
        else:
